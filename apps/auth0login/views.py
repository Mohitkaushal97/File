...
class auth0_sample_class_based_view(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(auth0_sample_class_based_view, self).get_context_data(**kwargs)
...
# https://docs.djangoproject.com/en/3.0/topics/class-based-views/intro/#decorating-the-class
class auth0_sample_class_user_status(TemplateView):
    template_name = 'auth0/dashboard.html'
    ...
    def get_context_data(self, **kwargs):
        context = super(auth0_sample_class_user_status, self).get_context_data(**kwargs)
        context['auth0User'] = 'auth0User'
        request = self.request
        user = request.user
        # auth0user = user.social_auth.get(provider='auth0')
...
def auth0_sample_login(request):
    return render(request, 'auth0/index.html')

def auth0_sample_logout(request):
    log_out(request)
