...
function remove_param_from_url(url, param_to_remove) {
    var r = url.split("?");
    if (r.length >= 2) {
        for (var f = r.shift(), e = r.join("?"), o = encodeURIComponent(t) + "=", i = e.split(/[&;]/g), u = i.length; u-- > 0; ) -1 !== i[u].lastIndexOf(o, 0) && i.splice(u, 1);
        url = f + "?" + i.join("&");
    }
    return url;
}
